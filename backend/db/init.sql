...
-- Movies table with improved constraints
CREATE TABLE IF NOT EXISTS movies (
  id SERIAL PRIMARY KEY,
  title VARCHAR(255) NOT NULL,
  director VARCHAR(255) NOT NULL,
  release_year INT NOT NULL 
    CHECK (release_year BETWEEN 1888 AND EXTRACT(YEAR FROM NOW()) + 5),
  description TEXT,
...
-- Screenings table with improved time handling
...
-- Seats table with improved numbering
...
-- Users table (should exist before bookings)
...
-- Triggers for all tables needing updated_at
CREATE TRIGGER update_movies_modtime
BEFORE UPDATE ON movies FOR EACH ROW EXECUTE FUNCTION update_modified_column();

CREATE TRIGGER UPDATE_bookings_modtime
BEFORE UPDATE ON bookings FOR EACH ROW EXECUTE FUNCTION update_modified_column();
...
-- Initial data with conflict handling
INSERT INTO movies (title, director, release_year, duration_minutes) VALUES
